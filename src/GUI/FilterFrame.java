...
public class FilterFrame {
	...
	private void initialize() {
		...
		btnApplyFilter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for(int i=0;i<FilteredDatabase.getSamples().size();i++)
					Database.getSamples().add(new WiFi(FilteredDatabase.getSamples().elementAt(i)));
	...
	private void UseFilter(Filter f)
	{
		for(int i=0;i<Database.getSamples().size();i++)
			if(f.test(Database.getSamples().elementAt(i)))
				FilteredDatabase.getSamples().add(new WiFi(Database.getSamples().elementAt(i)));
	}
